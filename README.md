...
#### IProductList
Описывает структуру данных, получаемую от сервера при запросе списка товаров

Интерфейс:
`interface IProductList {}` - определяет структуру ответа со списком товаров

Поля интерфейса:
`total: number` - количество товаров на сервере
`items: IProduct[]` - массив объектов товаров

#### IOrder
Описывает данные заказа, которык пользователь отправляет на сервер при покупке

Интерфейс:
`type IOrder = IBuyer & {}` - объединяет данные покупателя и информацию о заказе

Поля интерфейса:
`items: string[]` - массив товаров, добавленных в заказ
`total: number` - итоговая сумма заказа

#### IOrderResalt
Описывает структуру данных, которую сервер присылает после успешного оформления

Интерфейс:
`interface IOrderResalt {}` - определяет структуру ответа о покупке
...
`changes(): void {}` - уведомление об изменении даннвх покупателя 
...
Поля класса:
`_api: IApi` - экземпляр Api клиента

Методы класса:
`getProducts(): Promise<IProductList> {}` - Делает get запрос на эндпоинт `/product/` и возвращает объект, полученный от сервера, в котором находится массив товаров
`createOrder(order: IOrder): Promise<IOrderResalt> {}` - Делает post запрос на эндпоинт `/order/` и передаёт в него данные, полученные в параметрах метода, а возвращает объект, подтверждающий покупку на определенную сумму
...
Конструктор:
`constructor(container: HTMLElement, actions?: ICardBasketActions) {}` - принимает объект контейнер карточки и опционально объкет с действиями
